...
### Bring2Bring! (Already Wired Up): `bring2bring.deine-domain.de`

**Bring2Bring!** is a private digital cookbook (own repo, Node + SQLite) and
runs as a fourth dynamic app alongside winecashing, ginperium, and
tipsy-trails. The wiring (service in `docker-compose.yml`, block in
`config/caddy/Caddyfile`, entry in `sites.conf`) is already done - listed
here are only the points that need extra attention before the **first**
deploy:

- **One time, before the first deploy:** create the data directory and hand
  it to the container user:
  ```bash
  mkdir -p ~/pi-server/data/bring2bring && sudo chown -R 1000:1000 ~/pi-server/data/bring2bring
  ```
  Reason: the bind mount `./data/bring2bring:/data` covers up whatever the
  image prepared during the build; if Docker creates the directory itself
  (because it did not exist beforehand), it belongs to `root`. Bring2Bring!,
  however, runs unprivileged as the `node` user (UID 1000) and would then be
  unable to create its SQLite database. The other apps do not have this
  problem in the same form - see their respective Dockerfiles.
- **`PUBLIC_BASE_URL` lives in `docker-compose.yml`**, not in
  `apps/bring2bring/.env` - `prepare_site()` in `scripts/deploy.sh`
  completely rewrites this `.env` file on **every** deploy (only
  `SESSION_SECRET`/`ADMIN_USER`/`ADMIN_PASSWORD`); a manually added
  `PUBLIC_BASE_URL` there would silently disappear again on the next deploy,
  and the container would then fail to start afterwards (Bring2Bring! aborts
  hard on startup without this variable).
- **Cloudflare caution around `/r/*`:** this route is the deliberately public
  recipe-sharing page - Bring! fetches it from its own servers with a
  non-browser user agent. If `bring2bring.deine-domain.de` ever ends up
  behind Cloudflare bot fighting or an Access rule, every Bring! import
  breaks silently, while the site still looks completely normal in a
  browser.
...
- **Deploying:** `bash scripts/deploy.sh` clones/pulls Bring2Bring!, writes
  the admin `.env`, builds the container, runs `npm run seed:admin`, and
  reloads Caddy - same as the other admin apps.
